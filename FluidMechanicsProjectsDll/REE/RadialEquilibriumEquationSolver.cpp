...
{// initialize to default (as of testcase 1: free-vortex design)
...
bool REESolver::updateStreamField2()
{
...
#if _DEBUG
	std::ofstream myFile;
	myFile.open("./REE/output/check_A_matrix.csv");
	if (myFile.is_open()) {
		for (int ii = 0; ii <n; ii++) {
...
bool REESolver::writeReport()
{
	double Cm(-1.); 
	std::ofstream myFile;
	myFile.open("C://FluidMechProjects_source//FluidMechanicsProjectsDll//REE//output//REE-Report.csv");
	if (myFile.is_open()) {
